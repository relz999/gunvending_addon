...
include("shared.lua")
-- Set intial values
util.AddNetworkString("OwnerMenu")
...
function ENT:StartTouch(ent)
	...
	if ent:GetClass() == "spawned_shipment" and self.isVending == false then
		...
		local GunModel = CustomShipments[ent:Getcontents()].model
		local GunType = CustomShipments[ent:Getcontents()].entity
		self:SetShipmentSize(ShipmentSize)
		self:SetGunPrice(GunPrice)
		self:SetGunSellPrice(GunPrice)
		self:SetGunName(GunName)
		self:SetGunType(GunType)
		self:SetGunModel(GunModel)
		self:SetShipmentSizeOrig(ShipmentSize)
...
if caller:canAfford(self:GetGunPrice()) then			
	if self:GetShipmentSize() > 0 then
		...
		caller:addMoney(-self:GunSellPrice())
		self.Profit = self.Profit + self:GunSellPrice()
		...
		-- Pay Owner of gun vending machine
		owner:addMoney(self:GunSellPrice())
